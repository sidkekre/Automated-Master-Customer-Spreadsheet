...
docusign_connect = docusign.DocuSignConnectHandler(envelope_db)
...
def docusign_notification_received():
    if not HMAC_SECRET:
        ErrorLogger('DOCUSIGN_HMAC_SECRET is missing')
        return jsonify({'error': 'docusign_hmac_secret_missing'}), 500
